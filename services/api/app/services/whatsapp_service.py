...
def handle_incoming_message(payload: WhatsappIncoming) -> dict:
    ...
    if "pagar" in msg_lower or "qué tengo que pagar" in msg_lower or "que tengo que pagar" in msg_lower:
        pending = get_pending_payments_by_user(user["id"])
        if not pending:
            send_whatsapp_message(user["phone"], "No tienes pagos pendientes. ¡Todo al día!")
        else:
            response_lines = ["Tienes los siguientes pagos pendientes:"]
            for p in pending:
                svc = next((s for s in services if s["id"] == p["service_id"]), None)
                svc_name = svc["name"] if svc else "Servicio"
                response_lines.append(f"- {svc_name}: ${p['amount']} vence el {p['due_date']} (Ref: {p['id']})")
            send_whatsapp_message(user["phone"], "\n".join(response_lines))
    else:
        send_whatsapp_message(user["phone"], "No entendí tu mensaje. Puedes preguntar 'qué tengo que pagar'.")
